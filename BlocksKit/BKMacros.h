...
#define BK_EACH(collection, ...) __BK_EACH_WRAPPER([collection bk_each:^(id obj) { __VA_ARGS__ }];
#define BK_APPLY(collection, ...) __BK_EACH_WRAPPER([collection bk_apply:^(id obj) { __VA_ARGS__ }];
...
#define BK_REDUCE(collection, initial, ...) __BK_EACH_WRAPPER([collection bk_reduce:(initial) block:^id (id a, id b) { return (__VA_ARGS__); }])
