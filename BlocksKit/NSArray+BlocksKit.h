...
/**
 ...
 and reduce one of the specificated property of each object which is basic type.
 ...
 NSArray *peoples = @[p1, p2, p3];
 Integer totalAge = [peoples reduceInteger:0 WithBlock:^integer(NSInteger result, id obj, NSInteger index) {
    return result + [obj age];
 }];

 */
- (NSInteger)reduceInteger:(NSInteger)initial withBlock:(BKAccumulationBlockInteger)block;
...
/**
 ...
 For instance, if we want to caculate the total balance of a list of people.
